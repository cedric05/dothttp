...
from dothttp.parse import AWS4Auth
...
class AwsAuthUnit(TestBase):
    def base(
        ...
        self.assertEqual(
            access_id, auth.access_id, "access_id is different from expected"
        )
        return request
    ...
    def test_resolve_properties(self):
        ...
        self.base(
            params=dict(target="resolve properties", properties=prop), **properties
        )
    ...
    def test_default_region(self):
        self.base(
            params=dict(
                target="default us-east-1 region optional",
            ),
        )
    ...
    def test_no_service_and_no_region(self):
        self.base(
            params=dict(target="service from url and region defaulted to us-east-1")
        )
    ...
    def test_auth_headers_post(self):
        reqcomp = self.base(
            params=dict(target="with-x-amz-date header with post data"), service="ecr"
        )
        request = reqcomp.get_request()
        self.assertEqual(
            {
