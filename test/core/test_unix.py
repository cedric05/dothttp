...
from dothttp.parse.request_base import CurlCompiler, RequestCompiler
...
@skipIf(
    requests_unixsocket is None or not sys.platform.startswith("linux"),
    "in wasm mode, it will not be available",
)
class TestUnixSocketRequests(TestBase):
    def test_simple(self):
        request = self.get_request(file=f"{base_dir}/unix.http")
    ...
    def test_integration(self):
        ...
        with UnixSocketServerThread() as usock_thread:
            ...
            self.assertEqual(
                base_url + "/", response.url, "should be same request unix socket url"
            )
            self.assertEqual(200, response.status_code, "should be 200")
            self.assertEqual(
                "waitress", response.headers["server"], "server should be waitress"
            )
            self.assertEqual(
                usock_thread.usock,
                response.headers["X-Socket-Path"],
                "unix socket path should be same",
            )
    ...
    def test_url_extend_and_query(self):
        ...
        with UnixSocketServerThread() as usock_thread:
            ...
            self.assertEqual(
                "waitress", response2.headers["server"], "server should be waitress"
            )
            self.assertEqual(
                usock_thread.usock,
                response2.headers["X-Socket-Path"],
                "unix socket path should be same",
            )
            self.assertEqual(
                "/containers/nginx/logs/",
                response2.headers["X-Requested-Path"],
                "path should be same",
            )
            self.assertEqual(
                "timestamp=true", response2.headers["X-Requested-Query-String"]
            )

    def test_curl(self):
        request: CurlCompiler = self.get_req_comp(
            file=f"{base_dir}/unix.http", curl=True
        )
        self.assertEqual(
            """curl -X GET --url http://localhost/info \\
--unix-socket /var/run/docker.sock""",
            request.get_curl_output(),
        )
    ...
    def test_curl_with_query_and_path(self):
        ...
        self.assertEqual(
            """curl -X GET --url http://localhost/containers/nginx/logs/?timestamp=true \\
--unix-socket /var/run/docker.sock""",
            request.get_curl_output(),
        )
