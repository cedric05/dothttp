...
class ToHarTest(TestBase):
    ...
    def test_basic_content(self):
        self.basic_test(file=None, content="GET http://{{host=localhost:8000}}/get")
        ...
        linux = {
            "target": {
                "text": {
                    "headers": [],
                    "method": "POST",
                    "payload": {"mimeType": mimetype, "text": "raw data"},
                    "query": [],
                    "url": "https://req.dothttp.dev",
                }
            }
        }
        self.assertEqual(linux, self.execute_payload(target="text", **kwargs).result)
        filename = f"{command_dir}/payload.http"
        self.assertEqual(
            {
                "target": {
        ...
        with tempfile.NamedTemporaryFile(delete=False) as f:
            ...
            self.assertEqual(
                ...
                self.execute_payload(
                    target="fileinput", file=filename, fileinputarg=f.name
                ).result,
            )
    ...
    def test_aws_auth(self):
        ...
        with tempfile.NamedTemporaryFile(delete=False) as f:
            ...
            result = self.execute_payload(
                target="1", file=filename, fileinputarg=f.name
            ).result
            self.assertEqual(
                "GET",
                result["target"]["1"]["method"],
            )
            self.assertEqual(
            ...
            for headerData in result["target"]["1"]["headers"]:
                ...
                self.assertTrue(
                    ...
                    or name.lower().startswith("authorization"),
                    "header should be x-amz or authorization",
                )
                if headerData["name"].lower() == "authorization":
                    self.assertTrue(
                        "/eu-east-1/s3/aws4_request, SignedHeaders=host;x-amz-content-sha256;x-amz-date, Signature="
                        in headerData["value"]
                    )
    ...
    def execute_payload(self, fileinputarg="", **kwargs):
        kwargs["properties"] = {
            "filename": os.path.join(dir_path, f"{command_dir}/payload.http"),
            "fileinput2": fileinputarg,
        }
        command = Command(method=Http2Har.name, params=kwargs, id=1)
        result = self.execute_handler.run(command=command)
