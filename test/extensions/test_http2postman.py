...
class Testhttp2postman(TestBase):
    ...
    def test_simple(self):
        self.execute_n_get(
            ...
            input_file=os.path.join(
                command_dir,
                "swagger2har_petstore_response.http"))
    ...
    def test_payload(self):
        ...
        """
        self.execute_n_get(
            "payload.postman_collection.json",
            os.path.join(
                command_dir,
                "payload.http"))
    ...
    def test_negative(self):
        self.assertEquals({'error': True,
                           'error_message': 'filename not existent or invalid link'},
    ...
    def test_export_varibles(self):
        self.execute_n_get(
            http2postman_dir.joinpath(
                "withenv", "exportdothttpenv.postman_collection.json"), str(
                http2postman_dir.joinpath(
                    "withenv", "exportdothttpenv.http")), )

    def test_folder(self):
        self.execute_n_get(
            http2postman_dir.joinpath(
                "folderupload", "examples.postman_collection.json"), str(
                http2postman_dir.joinpath(
                    "folderupload", "examples")), )
