...
class HistoryValidate(TestBase):
    ...
    def test_execute_content(self):
        ...
        self.assertEqual(
            "http://localhost:8000/status/301", result.result["history"][0]["url"]
        )
        self.assertEqual(302, result.result["history"][1]["status"])
        self.assertEqual(
            "http://localhost:8000/redirect/1", result.result["history"][1]["url"]
        )
    ...
    def test_execute_content2(self):
        ...
        self.assertEqual(
            "http://localhost:8000/status/302", result.result["history"][0]["url"]
        )
        self.assertEqual(302, result.result["history"][1]["status"])
        self.assertEqual(
            "http://localhost:8000/redirect/1", result.result["history"][1]["url"]
