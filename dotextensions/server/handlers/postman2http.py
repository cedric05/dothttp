...
class ImportPostmanCollection(BaseHandler):
    ...
    @staticmethod
    def import_requests_into_dire(items: Iterator[Items],
                                  directory: Path,
                                  auth: Optional[Auth],
                                  variable: Union[None,
                                                  List[Variable]],
        ...
        base_auth_http = None
        if auth:
            base_inherit_auth_wrap, lines = ImportPostmanCollection.get_auth_wrap(
                auth)
            base_auth_http = Http(
                namewrap=NameWrap(INHERIT_AUTH),
                urlwrap=UrlWrap(
                    method="GET",
                    url="https://"),
                payload=None,
                lines=lines,
                authwrap=base_inherit_auth_wrap,
                output=None,
                certificate=None,
        ...
        for leaf_item in items:
            try:
                onehttp = ImportPostmanCollection.import_leaf_item(
                    leaf_item, INHERIT_AUTH if auth else None)
                if onehttp:
        ...
        if not (len(collection.allhttps) == 0 or (
                len(collection.allhttps) == 1 and base_auth_http)):
            name = str(directory.joinpath(
                f'imported_from_collection.{filetype.file_exts[0]}'))
            ...
            if variable is not None:
                ...
                d[str(directory.joinpath(".dothttp.json"))] = json.dumps(
                    {"*": dothttp_json_environment})
        return d
    ...
    def import_leaf_item(item: Items, base_http_name) -> Union[Http, None]:
        ...
        if req.auth and (
                req.auth.type == AuthType.NOAUTH or req.auth.type == AuthType_2_1.NOAUTH):
            base_http_name = None
            ...
            for header in req.header:
                lines.append(
                    Line(
                        header=Header(
                            key=header.key,
                            value=slashed_path_to_normal_path(
                                header.value)),
                        query=None))
        ...
        if req.body:
            ...
            if formdata := req.body.formdata:
                ...
                for one_form in formdata:
                    ...
                    if (one_form.type == FormParameterType.FILE) or (
                            one_form.type == FormParameterType2_0.FILE):
                        # TODO according to type, you would want to normalize
                        # path
                        files.append(
                            MultiPartFile(
                                one_form.key,
                                slashed_path_to_normal_path(
                                    one_form.src),
                ...
                if is_json_payload or (
                    optins and optins.get(
                        'raw',
                        None) and optins.get('raw').get(
                        'language',
                        None) and optins.get('raw').get('language') == 'json'):
                    try:
                ...
                for one_form_field in urlencoded_body:
                    encodedbody[one_form_field.key].append(
                        one_form_field.value)
                payload_datajson = encodedbody
            elif req.body.graphql:
                # TODO currently not supported
                pass
            payload = Payload(
                datajson=payload_datajson,
                data=payload_data,
                fileswrap=payload_fileswrap,
                json=payload_json,
                file=payload_file,
                type=payload_type)
        certificate = None
        if req.certificate and req.certificate.cert:
            certificate = Certificate(
                req.certificate.cert.src,
                req.certificate.key.src)
        http = Http(
            namewrap=namewrap,
            urlwrap=urlwrap,
            payload=payload,
            ...
            output=None,
            certificate=certificate,
            description=ImportPostmanCollection.extract_description(
                req.description))
        return http
    ...
    def get_auth_wrap(request_auth):
        ...
        if request_auth:
            ...
            if basic_auth := request_auth.basic:
                # postman 2.1, it is a list of api_key_element have keys and
                # values
                basic_auth = ImportPostmanCollection.api_key_element_to_dict(
                    basic_auth)
                # in postman 2.0, it is a dict
                auth_wrap = AuthWrap(
                    basic_auth=BasicAuth(
                        username=basic_auth.get(
                            'username', ''), password=basic_auth.get(
                            'password', '')))
            elif digest_auth := request_auth.digest:
                # postman 2.1, it is a list of api_key_element have keys and
                # values
                digest_auth = ImportPostmanCollection.api_key_element_to_dict(
                    digest_auth)
                # postman 2.0
                auth_wrap = AuthWrap(
                    digest_auth=DigestAuth(
                        username=digest_auth.get(
                            'username', ''), password=digest_auth.get(
                            'password', '')))
            elif ntlm_auth := request_auth.ntlm:
                # postman 2.1, it is a list of api_key_element have keys and
                # values
                ntlm_auth = ImportPostmanCollection.api_key_element_to_dict(
                    ntlm_auth)
                # postman 2.0
                auth_wrap = AuthWrap(
                    ntlm_auth=NtlmAuthWrap(
                        username=ntlm_auth.get(
                            'username', ''), password=ntlm_auth.get(
                            'password', '')))
            elif hawk_auth := request_auth.hawk:
                hawk_auth = ImportPostmanCollection.api_key_element_to_dict(
                    hawk_auth)
                # postman 2.0
                auth_wrap = AuthWrap(
                    hawk_auth=HawkAuth(
                        hawk_id=hawk_auth.get(
                            'authId', ''), key=hawk_auth.get(
                            'authKey', ''), algorithm=hawk_auth.get(
                            'algorithm', '')))
            elif request_auth.apikey:
                d = ImportPostmanCollection.api_key_element_to_dict(
                    request_auth.apikey)
                key = d.get('key', '<key>')
                ...
                aws_auth = ImportPostmanCollection.api_key_element_to_dict(
                    aws_auth)
    ...
    @staticmethod
    def api_key_element_to_dict(
            api_key_elements: Union[List[ApikeyElement], Dict]):
    ...
    def run(self, command: Command) -> Result:
        ...
        filetype = HttpFileType.get_from_filetype(
            command.params.get("filetype", None))
        ...
        if save:
            if not os.path.isdir(directory):
                return Result(
                    id=command.id,
                    result={
                        "error_message": f"non existent directory: {directory}",
                        "error": True})
            ...
            if not os.path.isabs(directory):
                return Result(
                    id=command.id,
                    result={
                        "error_message": "expects absolute path, as server is meant to run in background",
                        "error": True})
        # if not (link.startswith("https://www.postman.com/collections/") or link.startswith(
        #         "https://www.getpostman.com/collections")):
        # return Result(id=command.id, result={"error_message": "not a postman
        # link", "error": True})
        ...
        if "info" in postman_data and 'schema' in postman_data['info']:
            if postman_data['info']['schema'] == POSTMAN_2:
                ...
                base_collection_dire = Path(directory).joinpath(
                    clean_filename(collection.info.name))
                ...
                return Result(
                    id=command.id,
                    result={
                        "error_message": "unsupported postman collection",
                        "error": True})
            d = self.import_items(
                collection.item,
        ...
        if save:
            if collection.info.description:
                ...
                with open(base_collection_dire.joinpath("README.txt"), 'w') as f:
                    f.write(
                        ImportPostmanCollection.extract_description(
    ...
    def import_items(
        ...
        d = dict()
        d.update(ImportPostmanCollection.import_requests_into_dire(
            leaf_folder, directory, auth, variable, filetype, link))
        folder = map(
            lambda item: (
                item.name, item.item, item.auth), filter(
                lambda item: item.item, items))
        for sub_folder, subitem, item_auth in folder:
            d.update(
                ImportPostmanCollection.import_items(
                    subitem,
                    directory.joinpath(
                        clean_filename(sub_folder)),
                    item_auth or auth,
                    variable,
                    filetype,
                    link))
