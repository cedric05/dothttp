...
from dothttp.models.parse_models import (
    AwsAuthWrap,
    HawkAuth,
    HttpFileType,
    NtlmAuthWrap,
...
from ..postman import Items, URLClass, Variable, postman_collection_from_dict
from ..postman2_1 import POSTMAN_2_1, ApikeyElement
from ..postman2_1 import AuthType as AuthType_2_1
from ..postman2_1 import FormParameterType
from ..postman2_1 import URLClass as URLClass_2_1
from ..postman2_1 import postman_collection21_from_dict
from ..utils import clean_filename, get_alternate_filename, slashed_path_to_normal_path
from . import logger
...
class ImportPostmanCollection(BaseHandler):
    ...
    def import_requests_into_dire(
        ...
        if auth:
            ...
            base_auth_http = Http(
                ...
                urlwrap=UrlWrap(method="GET", url="https://"),
                payload=None,
                lines=lines,
        ...
        if not (
            len(collection.allhttps) == 0
            or (len(collection.allhttps) == 1 and base_auth_http)
        ):
            name = str(
                directory.joinpath(f"imported_from_collection.{filetype.file_exts[0]}")
            )
            data = HttpFileFormatter.format(collection, filetype)
            if HttpFileType.Httpfile == filetype:
                newline = os.linesep
                d[name] = (
                    f"#!/usr/bin/env dothttp{newline}{newline}"
            ...
            if req.url.query:
                for query in req.url.query:
                    if query.key is not None and query.value is not None:
                        lines.append(
                            Line(
                                query=Query(query.key, unquote(query.value)),
                                header=None,
                            )
                        )
        else:
            ...
            for header in req.header:
                lines.append(
                    Line(
                        header=Header(
                            key=header.key,
                            value=slashed_path_to_normal_path(header.value),
                        ),
                        query=None,
                    )
                )
                if (
                    header.key.lower() == "content-type"
                    and header.value.lower().startswith(APPLICATION_JSON)
                ):
        ...
        if req.body:
            ...
            if formdata := req.body.formdata:
                ...
                for one_form in formdata:
                    ...
                    if (one_form.type == FormParameterType.FILE) or (
                        ...
                        files.append(
                            MultiPartFile(
                                one_form.key,
                                slashed_path_to_normal_path(one_form.src),
                        ...
                        files.append(
                            MultiPartFile(
                                one_form.key, one_form.value, one_form.content_type
                            )
                        )
                ...
                if is_json_payload or (
                    optins
                    and optins.get("raw", None)
                    and optins.get("raw").get("language", None)
                    and optins.get("raw").get("language") == "json"
                ):
                    try:
            ...
            if basic_auth := request_auth.basic:
                ...
                auth_wrap = AuthWrap(
                    digest_auth=DigestAuth(
                        username=digest_auth.get("username", ""),
                ...
                auth_wrap = AuthWrap(
                    ntlm_auth=NtlmAuthWrap(
                        username=ntlm_auth.get("username", ""),
                ...
                lines.append(Line(header=header, query=None))
    ...
    def import_items(
        ...
        d = dict()
        d.update(
            ImportPostmanCollection.import_requests_into_dire(
                leaf_folder, directory, auth, variable, filetype, link
            )
        )
        folder = map(
            lambda item: (item.name, item.item, item.auth),
            filter(lambda item: item.item, items),
        )
